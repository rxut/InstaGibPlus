// ===============================================================
// Stats.ST_UT_FlakCannon: FlakCannon with Det deterministic fire
// ===============================================================
...
simulated function bool DetProcessStep(
	...
	if (AmmoType != none && AmmoType.AmmoAmount > 0) {
		...
		if (bAlt)
			BP.IGPlus_DetNoteShot(StepTS, 0.34);
		else
			BP.IGPlus_DetNoteShot(StepTS, 0.38);
		if (bServerSide)
...
function HandleDetServerFire(bool bAlt, rotator StepView, vector StepLoc) {
	...
	// (see ST_ShockRifle.HandleDetServerFire).
	if (bAlt) {
...
function SpawnServerChunks()
{
	...
	SpawnServerChunksAt(Start, AimRot);
}

function SpawnServerChunksAt(vector Start, rotator AimRot)
{
	local Vector X, Y, Z;
	local vector R;
	local Bot B;
...
function SpawnServerSlug()
{
	...
	SpawnServerSlugAt(Start, AimRot, PawnOwner.ViewRotation);
}

function SpawnServerSlugAt(vector Start, rotator AimRot, rotator OffsetRot)
{
	local Vector X, Y, Z;
