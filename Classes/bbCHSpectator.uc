...
// MUST stay in sync with ServerSettings.EShowDamageNumberMode declaration order.
...
replication
{
	...
	reliable if ( Role == ROLE_Authority )
		...
		IGPlus_WeaponSettingsDone; //, xxClientActivateMover;
...
event PostBeginPlay()
{
	...
	if (Role == ROLE_Authority && zzUTPure != none && zzUTPure.Settings != none)
...
event PostRender( canvas Canvas )
{
	...
	if (Settings.bSpectatorShowDamageNumbers)
		class'bbPlayerStatics'.static.DrawDamageNumbers(Canvas, Settings, LastDeltaTime);
}
...
exec function ShowDamageNumbers() {
	Settings.bSpectatorShowDamageNumbers = !Settings.bSpectatorShowDamageNumbers;
	Settings.SaveConfig();
	if (Settings.bSpectatorShowDamageNumbers) {
		if (ShowDamageNumberMode == DMG_Disabled)
			ClientMessage("Spectator damage numbers: ON (server has damage numbers disabled - no numbers will appear)", 'IGPlus');
		else
			ClientMessage("Spectator damage numbers: ON", 'IGPlus');
	} else {
		ClientMessage("Spectator damage numbers: OFF", 'IGPlus');
	}
}
