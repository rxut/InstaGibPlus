...
function ServerExplicitFire(vector ClientLoc, rotator ClientRot, int NumRockets, bool bAlt)
{
	local PlayerPawn P;
...
// Override Fire/AltFire to prevent server-side logic loop when ping comp is enabled
...
simulated function FiringRockets()
{
    ...
    bbP = bbPlayer(Owner);
    if (Role < ROLE_Authority && bbP != None && GetWeaponSettings().bEnablePingCompensation)
    {
        ...
        ServerExplicitFire(Pawn(Owner).Location, Pawn(Owner).ViewRotation, ClientRocketsLoaded, bAlt);
    }
}
...
state FireRockets
{
	...
	function BeginState()
	{
		...
		// Fix for TightWad not triggering when AltFire is held
		if ( PawnOwner.bAltFire != 0 )
			bTightWad = true;
